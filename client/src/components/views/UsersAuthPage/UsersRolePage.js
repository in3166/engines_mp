...
import { Layout, Breadcrumb, Table, Space, Button, message, Popconfirm, Modal, Select,Spin ,Form} from 'antd';
...
function UsersRolePage(props) {
    ...
    // 수정 모달 OK 버튼 - redux
    const modalOnOk = () =>{
        let newRole = modalData.role === '일반 사용자' ? 0 
                        : modalData.role === '전문가' ? 2 : 3 ;
    ...
    const deleteConfirm = (user) => {
        ...
        axios.post('/api/users/deleteUser', body)
        .then(res=> {
            if(res.data.success){
                ...
                >   
                    <Spin spinning={loading}>
                        <div style={{float:'right'}}>
                            <Button onClick={getAllUsers}>새로고침</Button>  <Button onClick={deleteUsersButton}>회원 탈퇴</Button> <br/><br/>
                        </div>
                    ...
                    >
                        <Form {...{labelCol: { span: 6 }, wrapperCol: { span: 14 }}} name="userinfo-change">
                            <Form.Item label="ID">
                            ...
                            <Form.Item label="권한">
                                <Select value={modalData.role}  style={{ width: 120 }}  onChange={value=>handleRoleChange(value)}>
